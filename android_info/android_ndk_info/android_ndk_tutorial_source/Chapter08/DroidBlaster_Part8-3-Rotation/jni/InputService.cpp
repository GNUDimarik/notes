...
namespace packt {
    InputService::InputService(android_app* pApplication,
        ...
        mMenuKey(false),
        mRotation(pConfiguration->getRotation()),
    ...
    bool InputService::onAccelerometerEvent(ASensorEvent* pEvent) {
        ...
        ASensorVector lVector;
        toScreenCoord(mRotation, &pEvent->vector, &lVector);

        // Roll tilt.
        float lRawHorizontal = lVector.x / GRAVITY;
        ...
        float lRawVertical = lVector.z / GRAVITY;
    ...
    // From http://developer.download.nvidia.com/tegra/docs/tegra_android_accelerometer_v5f.pdf
