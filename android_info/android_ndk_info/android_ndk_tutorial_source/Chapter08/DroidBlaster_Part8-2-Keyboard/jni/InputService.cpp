...
namespace packt {
    ...
    status InputService::start() {
        ...
        mHorizontal = 0.0f, mVertical = 0.0f;
        mMenuKey = false; mEKey = false;
        if ((mWidth == 0) || (mHeight == 0)) {
            return STATUS_KO;
        }
        
        if (mKeyboard->initialize() != STATUS_OK) return STATUS_KO;
        return STATUS_OK;
    }

    status InputService::update() {
        // Displays keyboard if requested.
        if (mMenuKey) {
            mKeyboard->show();
        }
        if (mEKey) {
